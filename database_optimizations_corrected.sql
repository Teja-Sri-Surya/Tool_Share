-- =====================================================
-- ToolShare Database Optimizations (Corrected)
...
ALTER TABLE api_user ADD COLUMN latitude DECIMAL(10, 8) DEFAULT NULL;
ALTER TABLE api_user ADD COLUMN longitude DECIMAL(11, 8) DEFAULT NULL;
ALTER TABLE api_user ADD COLUMN location_updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP;

-- Add location columns to tools table if not exists
ALTER TABLE api_tool ADD COLUMN latitude DECIMAL(10, 8) DEFAULT NULL;
ALTER TABLE api_tool ADD COLUMN longitude DECIMAL(11, 8) DEFAULT NULL;
ALTER TABLE api_tool ADD COLUMN location_updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP;

-- Create spatial indexes for geographic queries
CREATE INDEX idx_users_location ON api_user (latitude, longitude);
...
BEGIN
    IF OLD.status != NEW.status THEN
        IF NEW.status IN ('active', 'confirmed', 'pending') THEN
            ...
            UPDATE api_tool SET available = FALSE, updated_at = NOW() WHERE id = NEW.tool_id;
        ELSEIF NEW.status IN ('completed', 'cancelled', 'forfeited') THEN
            -- Make tool available
            UPDATE api_tool SET available = TRUE, updated_at = NOW() WHERE id = NEW.tool_id;
        END IF;
    END IF;
END //
DELIMITER ;
